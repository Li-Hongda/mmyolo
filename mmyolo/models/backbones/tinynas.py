...
class ResConvBlock(BaseModule):
    def __init__(self,
        ...
        if not reparam:
            self.conv2 = ConvModule(btn_channels, 
                                    out_channels, 
                                    kernel_size, 
                                    stride,
                                    norm_cfg=norm_cfg,
            ...
            self.conv2 = RepVGGBlock(btn_channels,
                                     out_channels,
                                     kernel_size,
                                     stride,
                                     norm_cfg=norm_cfg,
        ...
        if in_channels != out_channels and stride != 2:
            self.residual_proj = ConvModule(in_channels, 
                                            out_channels, 
                                            kernel_size=1, 
                                            stride=1,
                                            norm_cfg=norm_cfg,
...
class CSPWrapper(BaseModule):
    def __init__(self,
        ...
        self.downsampler = ConvModule(in_channels,
                                      hidden_channels * 2,
                                      kernel_size=3,
                                      stride=2,
                                      padding=1,
                                      norm_cfg=norm_cfg)
        if self.use_spp:
            self.spp = SPPBottleneck(hidden_channels * 2, 
                                     hidden_channels * 2,
                                     act_cfg=act_cfg)
        if len(self.convstem) > 0:
            self.conv_start = ConvModule(hidden_channels * 2,
                                         hidden_channels,
                                         kernel_size=1,
                                         stride=1,
                                         norm_cfg=norm_cfg)
            self.conv_shortcut = ConvModule(hidden_channels * 2,
                                            out_channels // 2,
                                            kernel_size=1,
                                            stride=1,
                                            norm_cfg=norm_cfg)
            self.conv_fuse = ConvModule(out_channels, 
                                        out_channels, 
                                        kernel_size=1,
                                        stride=1,
                                        norm_cfg=norm_cfg)
...
class TinyNAS(BaseBackbone):
    # in_channels, out_channels, btn_channels, kernel_size, stride, num_blocks, use_spp
    arch_settings = {
    ...
    def build_stage_layer(self, stage_idx: int, setting: list) -> list:
        if self.arch in ['S', 'T']:        
            in_channels, out_channels, btn_channels, kernel_size, stride, num_blocks, use_spp = setting
            # if the_block_class == 'SuperResConvK1KX':
            spp = use_spp if stage_idx == len(self.arch_settings[self.arch]) - 1 else False
            stage = []
            the_block = SuperResStem(in_channels,
                                    out_channels,
                                    btn_channels,
                                    kernel_size,
                                    stride,
            ...
            in_channels, out_channels, btn_channels, kernel_size, stride, num_blocks, extra_cfg = setting
            stage = []
            use_spp = True if stage_idx == len(self.arch_settings[self.arch]) - 1 else False
            if extra_cfg is not None:
                in_c, out_c, btn_c, k_size, s, n_blocks = extra_cfg
                block_1 = CSPStem(in_channels,
                                out_channels,
                                btn_channels,
                                kernel_size,
                                stride,
